...
export default function Budgets() {
  ...
  const handleDepositGoal = (e) => {
    ...
    triggerNotification(`Poupança adicionada! Depositou €${depositAmount} na meta "${goal.name}".`);
  };
              ...
              return (
                <div key={budget.id} style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      {isOver && (
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', fontSize: '0.7rem', color: 'var(--danger)', backgroundColor: 'var(--danger-glow)', padding: '0.1rem 0.4rem', borderRadius: '4px' }}>
                          <AlertOctagon size={10} /> Estourado em €{Math.abs(budget.spent - budget.limit).toFixed(2)}
                        </span>
                      )}
                      {isNear && (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {editingBudgetId === budget.id ? (
                        ...
                        <div style={{ fontSize: '0.85rem' }}>
                          ...
                          <span style={{ color: '#fff', fontWeight: '600' }}>€{budget.limit.toFixed(2)}</span>
                          <button 
                            className="btn-icon" 
                  ...
                  <div style={{ display: 'flex', justifySpace: 'space-between', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    ...
                    <span>Disponível: €{Math.max(0, budget.limit - budget.spent).toFixed(2)}</span>
                  </div>
                </div>
              );
            })}
                ...
                <div key={goal.id} className="goal-card" style={{ 
                  ...
                  <div className="goal-header">
                    ...
                    <span className="goal-values">
                      €{goal.current} <span style={{ color: 'var(--text-muted)' }}>/ €{goal.target}</span>
                    </span>
                  </div>
